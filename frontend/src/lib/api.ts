...
export interface Qingshu101DirectoryStatus {
  refreshed_at: number
...
export interface LimitLadderResult {
  as_of: string
  data_freshness?: {
    source: 'teajoin.daily' | 'persisted.enriched' | string
...
export const api = {
  ...
  // ===== Auth (访问认证) =====
  authStatus: () =>
    request<{ configured: boolean; authenticated: boolean; user: AuthUser | null }>('/api/auth/status'),
  authMe: () => request<{ user: AuthUser }>('/api/auth/me'),
  authEntry: (name: string, phone: string, password: string) =>
    request<{ ok: boolean; created: boolean; authenticated: boolean; user: AuthUser }>('/api/auth/entry', {
      method: 'POST',
  ...
  qingshu101Status: () =>
    request<{
      ...
      directory: Qingshu101DirectoryStatus | null
    }>('/api/qingshu101/status'),
  qingshu101CreateSession: (key: string) =>
    request<{ ok: boolean; expires_in: number }>('/api/qingshu101/session', {
      method: 'POST',
      body: JSON.stringify({ key }),
    }),
  qingshu101Logout: () =>
    request<{ ok: boolean }>('/api/qingshu101/logout', { method: 'POST' }),
  qingshu101Users: (offset = 0, limit = 100) =>
    request<{
      total: number
      offset: number
      limit: number
      refreshed_at: number
      directory_status: Qingshu101DirectoryStatus
      users: Qingshu101User[]
    }>(`/api/qingshu101/users?offset=${offset}&limit=${limit}`),
  ...
  financialSearch: (q: string, limit = 20) =>
    request<{ results: { symbol: string; name: string; code: string; asset_type?: string }[] }>(
      `/api/financials/search?q=${encodeURIComponent(q)}&limit=${limit}`,
    ),
