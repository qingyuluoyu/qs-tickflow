...
export function IndustryAnalysis() {
  ...
  const [membersTarget, setMembersTarget] = useState<DimensionMembersTarget | null>(null)
  ...
  const openMembers = (value: string) => {
    setSelectedKey(value)
    if (!activeConfig || !resolved.dimensionField) return
    setMembersTarget({
      kind: 'industry',
      value,
      sourceField: `${activeConfig.id}.${resolved.dimensionField}`,
      date: rowsQuery.data?.date ?? undefined,
    })
  }

  const openStockAnalysis = (stockSymbol: string, stockName?: string) => {
    ...
    return (
      <>
        <div className="flex h-full flex-col">
          ...
          <PresetFetchState
            title="暂无行业数据"
            hint="服务器正在预处理行业分类数据，完成后即可使用行业分析"
  ...
  return (
    <>
      <PageHeader
        ...
        right={
          <div className="flex items-center gap-1">
            ...
            <Tooltip label="行业涨幅轮动矩阵" position="bottom">
              <Button
                size="xs"
                variant="light"
                color="yellow"
      ...
      <div className="min-h-full bg-[radial-gradient(circle_at_12%_0%,rgba(245,158,11,0.12),transparent_28%),radial-gradient(circle_at_85%_8%,rgba(244,63,94,0.08),transparent_28%)]">
        <PageContainer className="mx-auto max-w-[1440px] space-y-5">
          ...
          <MarketPulse
            ...
            onStockClick={openStockAnalysis}
          />
          ...
          {stats.length > 0 ? (
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[18rem_1fr]">
              <IndustryRail
                ...
                onSelect={openMembers}
              />
              <IndustryFocus stat={selected} onStockClick={openStockAnalysis} />
            </div>
          ) : rowsQuery.isLoading ? (
      ...
      <DimensionMembersDialog
        ...
        onStockClick={openStockAnalysis}
        onAnalyzeClick={openStockAnalysis}
      />
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active ? cn('bg-warning/10', toneBorder) : cn('hover:bg-elevated/35', toneHover),
              )}
            >
    ...
    <section className="rounded-2xl border border-border bg-surface p-2.5">
      ...
      <div className="max-h-[620px] overflow-auto rounded-lg border border-border/50">
        {stats.map(item => {
          const active = selectedKey === item.key
          return (
            <button key={item.key} onClick={() => onSelect(item.key)} className={cn('w-full border-b border-border/50 px-2.5 py-2 text-left transition-colors last:border-b-0', active ? 'bg-warning/10' : 'hover:bg-elevated/40')}>
