...
export function AnalysisKChart({
  ...
  const zoomWindowRef = useRef<{ start: number; end: number } | null>(null)
  ...
  const chartHeight = isFullscreen
    ? Math.max(520, Math.min(900, viewportHeight - 170))
    : height
  ...
  const toggleFullscreen = async () => {
    ...
    if (shell.requestFullscreen) {
      try {
        ...
        // Fall back to a fixed overlay when browser policy blocks native fullscreen.
      }
    }
  ...
  const buildOption = (): EChartsOption => {
    ...
    const mainH = chartHeight - PAD_TOP - GAP_MAIN_VOL - volH - GAP_VOL_SLIDER - SLIDER_H - PAD_BOTTOM
  ...
  useEffect(() => {
    ...
    if (!chartInstRef.current) {
      ...
      chartInstRef.current.on('datazoom', () => {
        const option = chartInstRef.current?.getOption() as { dataZoom?: Array<{ start?: number; end?: number }> }
    ...
    if (zoom && Array.isArray(option.dataZoom)) {
      for (const dataZoom of option.dataZoom) {
        dataZoom.start = zoom.start
        dataZoom.end = zoom.end
      }
    }
    chartInstRef.current.setOption(option, true)
    chartInstRef.current.resize()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rows, levels, series, seriesDates, activeTypes, pivotRank, markers, ranges, chartHeight, theme, hoveredKey])
  ...
  return (
    <div
      ...
      className={[className, isFullscreen ? 'fixed inset-0 z-[100] overflow-y-auto bg-base p-5' : '']
        .filter(Boolean)
        .join(' ')}
    >
      {/* 价位开关按钮组 */}
