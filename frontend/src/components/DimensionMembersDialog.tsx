...
interface Props {
  ...
  /** Optional action shown in the header for navigating the selected member to stock analysis. */
  onAnalyzeClick?: (symbol: string, name?: string) => void
}
...
export function DimensionMembersDialog({ target, onClose, onStockClick, onAnalyzeClick }: Props) {
...
function DimensionMembersDialogContent({ target, onClose, onStockClick, onAnalyzeClick }: Omit<Props, 'target'> & { target: DimensionMembersTarget }) {
  ...
  // Dashboard drill-downs expose a header action. Select the first member so the
  // action is immediately usable, then keep the user's selection while filtering.
  useEffect(() => {
    if (!onAnalyzeClick) return
    setSelectedStock(current => {
      if (current && rows.some(row => row.symbol === current.symbol)) return current
      const first = rows[0]
      return first ? { symbol: first.symbol, name: first.name ?? undefined } : null
    })
  }, [rows, onAnalyzeClick])
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          {onAnalyzeClick && (
            <button
              type="button"
              onClick={() => selectedStock && onAnalyzeClick(selectedStock.symbol, selectedStock.name)}
              disabled={!selectedStock}
              className="inline-flex h-7 items-center gap-1 rounded-btn border border-accent/35 bg-accent/10 px-2 text-[11px] font-medium text-accent transition-colors hover:bg-accent/20 disabled:cursor-not-allowed disabled:opacity-40"
              title={selectedStock ? `查看 ${selectedStock.name || selectedStock.symbol} 个股分析` : '先选择一只股票'}
            >
              个股分析
            </button>
          )}
                ...
                {rowVirtualizer.getVirtualItems().map(virtualRow => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        setSelectedStock({ symbol: row.symbol, name: row.name })
                        onStockClick?.(row.symbol, row.name)
                      }}
                      disabled={!onStockClick && !onAnalyzeClick}
                      className={`absolute left-0 top-0 grid min-h-[54px] w-full grid-cols-[minmax(132px,1fr)_74px_74px_18px] items-center border-b border-border/60 px-4 text-left text-xs transition-colors hover:bg-elevated/50 disabled:cursor-default md:grid-cols-[minmax(180px,1fr)_90px_84px_88px_100px_18px] ${selectedStock?.symbol === row.symbol ? 'bg-accent/10' : ''}`}
