...
test('后端底稿等待期间有连接状态，中止后立即解除运行态', () => {
  const dialogSource = readFileSync(new URL('../src/components/stock-analysis/DebateDialog.tsx', import.meta.url), 'utf8')

  assert.match(dialogSource, /正在连接后端/)
  assert.match(dialogSource, /abortRef\.current\?\.abort\(\)\s*\n\s*setRunning\(false\)/)
    assert.match(dialogSource, /event\.type === 'dossier_progress'/)
})
...
test('个股 AI 流支持 AbortSignal 且工作态提供停止入口', () => {
  ...
  assert.match(apiSource, /stockAnalyzeStream\(symbol: string, focus\?: string, signal\?: AbortSignal\)/)
