...
本项目通过 `MarketDataProvider` 统一接入数据。生产部署可把 TeaJoin 作为服务器级 provider；自定义数据源由外部 HTTP 服务负责取数和整理，本项目只把返回结果映射成内部标准字段，然后复用现有存储、指标、enriched、策略和前端展示逻辑。
...
| 分钟K | `minute` | 个股/ETF/指数分钟行情，支持 `asset_type` 和 `freq` 参数 |
| 财务 | `financial` | 财报指标、利润表、资产负债表、现金流和股本等表 |

深度盘口需要独立的 `DEPTH5_BATCH` 能力，不在此 YAML 契约内。

## TeaJoin 生产配置

仓库已提供 `data/data_sources/teajoin.yaml`，所有数据集均经过字段、日期、重复和单位标准化。令牌只放在环境变量或 `.env`：

```text
TEAJOIN_API_KEY=...
```

provider 选择写入服务器级 `data/server_config.json`，不会跟随某个用户工作区。示例：

```json
{
  "daily_data_provider": "teajoin",
  "adj_factor_provider": "teajoin",
  "minute_data_provider": "teajoin",
  "realtime_data_provider": "teajoin",
  "financial_data_provider": "teajoin",
  "realtime_quotes_enabled": true,
  "financial_schedule": {"enabled": false, "interval_days": 7}
}
```

实时行情由服务器后台按轮询间隔拉取；多账户自选会先做账户工作区并集，再一次性调用 TeaJoin，不依赖用户打开页面。`/api/intraday/status` 返回 `provider_health`、`snapshot_age_ms` 和失败分类。失败时保留最后一次有效快照，不回退到 TickFlow，也不填充默认零值。
...
如果服务器不方便配置环境变量，可把密钥作为服务器私有文件上传到数据目录（该目录已被忽略，不会进入源码包）：

```text
data/private/TEAJOIN_API_KEY
```

文件只放一行密钥并限制文件权限。读取优先级为：环境变量 → `data/private/<变量名>` → `data/secrets/<变量名>` → `data/.<变量名>` → `.env`。密钥不会写入日志、API 响应或前端代码；源码包仍可直接上传，服务器只需携带 `data/private` 私有目录。
