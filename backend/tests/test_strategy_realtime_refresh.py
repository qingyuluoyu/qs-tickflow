...
def test_account_alerts_are_delivered_only_to_the_matching_subscriber():
    service = QuoteService()
    first = service.subscribe(owner_id="user-a")
    second = service.subscribe(owner_id="user-b")

    service.push_alerts([{"symbol": "600000.SH"}], owner_id="user-a")

    assert first.pop()["alerts"] == [{"symbol": "600000.SH"}]
    assert second.pop()["alerts"] == []


def test_legacy_alerts_are_broadcast_to_all_subscribers():
    service = QuoteService()
    first = service.subscribe(owner_id="user-a")
    second = service.subscribe(owner_id="user-b")

    service.push_alerts([{"symbol": "000001.SZ"}])

    assert first.pop()["alerts"] == [{"symbol": "000001.SZ"}]
    assert second.pop()["alerts"] == [{"symbol": "000001.SZ"}]
