...
def test_fail_closed_custom_provider_without_minute_does_not_use_tickflow(monkeypatch):
    """A source explicitly marked fail-closed must not silently query TickFlow."""
    provider = SimpleNamespace(config=SimpleNamespace(fail_closed=True))
    monkeypatch.setattr(
        kline_sync.preferences,
        "get_minute_data_provider",
        lambda: "teajoin",
...
def test_fail_closed_custom_minute_provider_exception_does_not_use_tickflow(monkeypatch):
    provider = SimpleNamespace(
        config=SimpleNamespace(fail_closed=True),
        get_minute=lambda *_args, **_kwargs: (_ for _ in ()).throw(httpx.TimeoutException("timeout")),
    )
    _setup_custom_provider(monkeypatch, provider, has_dataset=True)
    monkeypatch.setattr(kline_sync.preferences, "get_minute_data_provider", lambda: "teajoin")
...
def test_sync_minute_single_runs_stock_sync(monkeypatch):
    import asyncio
    ...
    mock_request = MagicMock()
    mock_request.app.state.repo = mock_repo
    mock_request.app.state.capabilities = MagicMock()
    monkeypatch.setattr(kline_api, "_minute_allowed", lambda _capset: True)
    monkeypatch.setattr("app.services.preferences.get_minute_sync_days", lambda: 5)
    monkeypatch.setattr(kline_api.kline_sync, "sync_and_persist_minute", lambda *_args, **_kwargs: 7)
    monkeypatch.setattr("app.jobs.daily_pipeline._refresh_single_view", lambda *_args: None)

    result = asyncio.run(kline_api.sync_minute_single(mock_request, {"symbol": "000001.SZ"}))

    assert result == {"status": "ok", "symbol": "000001.SZ", "rows": 7}
