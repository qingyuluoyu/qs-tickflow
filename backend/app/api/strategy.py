...
def _get_engine(request: Request) -> StrategyEngine:
    try:
        ...
        return get_user_strategy_engine(request)
...
def _invalidate_strategy_runtime(request: Request) -> None:
    ...
    from app.services.strategy_runtime import invalidate_user_strategy_engine

    strategy_cache.clear_cache(_data_dir(request))
    invalidate_user_strategy_engine(request)
...
def _cleanup_deleted_strategy(request: Request, strategy_id: str) -> list[str]:
    ...
    try:
        ...
        monitor_engine = None if getattr(getattr(request, "state", None), "user", None) else getattr(request.app.state, "monitor_engine", None)
        if rules_changed and monitor_engine is not None:
            monitor_engine.set_rules(monitor_rules.load_all(data_dir))
        elif rules_changed and getattr(getattr(request, "state", None), "user", None) is not None:
            runtime = getattr(request.app.state, "monitor_runtime", None)
            if runtime is not None:
                runtime.invalidate(request.state.user.id)
    except Exception as e:
