...
def _rules_for_request(request: Request) -> tuple[list[dict], object | None]:
    """Read the caller's rules without borrowing another account's engine."""
    engine = getattr(request.app.state, "monitor_engine", None)
    if getattr(getattr(request, "state", None), "user", None) is not None:
        return monitor_rules.load_all(_data_dir(request)), None
    return (list(engine.rules.values()) if engine is not None else []), engine
...
def _sync_engine(request: Request) -> None:
    ...
    # The background QuoteService owns one process-level engine for legacy
    # single-user mode. Never replace that engine with one account's rules.
    if getattr(getattr(request, "state", None), "user", None) is not None:
        runtime = getattr(request.app.state, "monitor_runtime", None)
        user = getattr(request.state, "user", None)
        if runtime is not None and user is not None:
...
def save_rule(req: RuleModel, request: Request):
    ...
    if rule.get("type") == "strategy":
        ...
        try:
            strategy_engine = get_user_strategy_engine(request)
...
def test_ladder(request: Request):
    ...
    rules, _engine = _rules_for_request(request)

    if not depth_svc:
        raise HTTPException(status_code=503, detail="depth 服务未初始化")
    if not any(r.get("type") == "ladder" and r.get("enabled", True) for r in rules):
    ...
    ladder_rules = [r for r in rules if r.get("type") == "ladder" and r.get("enabled", True)]
    all_events = []
    not_triggered = []
...
def trigger_ladder(request: Request):
    ...
    repo = request.app.state.repo
    depth_svc = getattr(request.app.state, "depth_service", None)
    rules, webhook_engine = _rules_for_request(request)
    user = getattr(getattr(request, "state", None), "user", None)
    if user is not None:
        runtime = getattr(request.app.state, "monitor_runtime", None)
        if runtime is not None:
            webhook_engine = runtime.get(user.id)
    ...
    # 1. 落盘到 alerts.jsonl
    try:
        alert_store.append_many(_data_dir(request), rule_events)
    ...
    if quote_svc:
        ...
        try:
            user = getattr(getattr(request, "state", None), "user", None)
            quote_svc.push_alerts(sse_alerts, owner_id=user.id if user is not None else None)
        except Exception:  # noqa: BLE001
            pass
    ...
    if quote_svc:
        try:
            quote_svc._maybe_send_webhook(rule_events, webhook_engine)
        except Exception:  # noqa: BLE001
            pass
