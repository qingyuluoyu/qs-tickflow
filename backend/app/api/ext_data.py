...
def _store(request: Request) -> ExtConfigStore:
    return ExtConfigStore(_data_dir(request))
...
def _shared_data_dir(request: Request) -> Path:
    """Return the server-owned market-data root, never a user's workspace."""
    return Path(request.app.state.repo.store.data_dir)


def _config_and_data_dir(request: Request, config_id: str) -> tuple[ExtConfig | None, Path]:
    """Resolve built-in market datasets as shared and custom datasets as private."""
    data_dir = _shared_data_dir(request) if config_id in _BUILTIN_CONFIG_IDS else _data_dir(request)
    return ExtConfigStore(data_dir).get(config_id), data_dir
...
def list_configs(request: Request):
    """列出所有扩展数据配置。"""
    personal_root = _data_dir(request)
    shared_root = _shared_data_dir(request)
    personal = [
        config for config in ExtConfigStore(personal_root).load_all()
        if config.id not in _BUILTIN_CONFIG_IDS
    ]
    shared = [
        config for config in ExtConfigStore(shared_root).load_all()
        if config.id in _BUILTIN_CONFIG_IDS
    ]
    configs = [*shared, *personal]
    items = []
    for c in configs:
        data_dir = shared_root if c.id in _BUILTIN_CONFIG_IDS else personal_root
        d = c.to_dict()
        if c.id in _BUILTIN_CONFIG_IDS:
            # Upstream recipes are server implementation details. Clients only
            # receive the data contract and freshness, never provider URLs or
...
async def fetch_preset_data(request: Request, config_id: str):
    ...
    try:
        n = await fetch_preset(config_id, _shared_data_dir(request))
...
def list_rows(
    ...
    """
    config, data_dir = _config_and_data_dir(request, config_id)
    if not config:
        raise HTTPException(404, f"配置 '{config_id}' 不存在")
