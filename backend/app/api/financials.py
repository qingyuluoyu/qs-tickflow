...
@router.get("/search")
def search_financial(request: Request, q: str = Query("", max_length=50), limit: int = Query(20, ge=1, le=50)):
    """Search stocks for the financial page, falling back to TeaJoin instruments."""
    capset = request.app.state.capabilities
    _require_financial(capset)
    return {"results": search_financial_symbols(request.app.state.repo, q, limit)}
...
def financial_status(request: Request):
    ...
    schedule = server_preferences.get_financial_schedule() if fs else {}
...
def get_metrics(request: Request, symbol: str | None = None):
    ...
    if not symbol:
        ...
        df = load_financial_frame(request.app.state.repo.store.data_dir, "metrics", symbol)
    if df.is_empty():
        return {"data": []}
    if symbol and "symbol" in df.columns:
        df = df.filter(pl.col("symbol") == symbol.strip().upper())
...
def get_income(request: Request, symbol: str | None = None):
    ...
    if not symbol:
        ...
        df = load_financial_frame(request.app.state.repo.store.data_dir, "income", symbol)
    if df.is_empty():
        return {"data": []}
    if symbol and "symbol" in df.columns:
        df = df.filter(pl.col("symbol") == symbol.strip().upper())
...
def get_balance_sheet(request: Request, symbol: str | None = None):
    ...
    if not symbol:
        ...
        df = load_financial_frame(request.app.state.repo.store.data_dir, "balance_sheet", symbol)
    if df.is_empty():
        return {"data": []}
    if symbol and "symbol" in df.columns:
        df = df.filter(pl.col("symbol") == symbol.strip().upper())
...
def get_cash_flow(request: Request, symbol: str | None = None):
    ...
    if not symbol:
        ...
        df = load_financial_frame(request.app.state.repo.store.data_dir, "cash_flow", symbol)
    if df.is_empty():
        return {"data": []}
    if symbol and "symbol" in df.columns:
        df = df.filter(pl.col("symbol") == symbol.strip().upper())
...
def get_shares(request: Request, symbol: str | None = None):
    ...
    if not symbol:
        ...
        df = load_financial_frame(request.app.state.repo.store.data_dir, "shares", symbol)
    if df.is_empty():
        return {"data": []}
    if symbol and "symbol" in df.columns:
        df = df.filter(pl.col("symbol") == symbol.strip().upper())
