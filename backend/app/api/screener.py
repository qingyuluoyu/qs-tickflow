...
def _load_ext_value_maps(
    ...
    *,
    ...
    db = repo.store.db
    data_dir = Path(data_dir or repo.store.data_dir)
    ...
    for config_id, field_name in ext_specs:
        ...
        cache_key = (str(data_dir.resolve()), config_id, field_name)
...
def strategies(
    ...
    from app.services.strategy_runtime import get_user_strategy_engine
    ...
    try:
        engine = get_user_strategy_engine(request)
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail="策略引擎未初始化") from exc
    presets = []
...
def run_custom(req: CustomRequest, request: Request):
    ...
    from app.services.user_context import request_data_root
    data_dir = request_data_root(request, shared_root=repo.store.data_dir)
    ext_values = _load_ext_value_maps(repo, req.ext_columns, data_dir=data_dir)
...
def run_preset(req: PresetRequest, request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"策略引擎未初始化或策略 {req.strategy_id} 不存在") from exc
...
def _cached_with_realtime(request: Request) -> dict:
    ...
    # In account mode the process-level monitor engine must never be used as
    # a cross-user result cache. Personal monitor runtime is intentionally
    # read from the account workspace by its owning request only.
    user = getattr(getattr(request, "state", None), "user", None)
    if user is not None:
        runtime = getattr(request.app.state, "monitor_runtime", None)
        monitor_engine = runtime.get(user.id) if runtime is not None else None
    else:
...
def get_cached(
    ...
    repo = request.app.state.repo
    data_dir = request_data_root(request, shared_root=repo.store.data_dir)
    ext_values = _load_ext_value_maps(repo, ext_columns, data_dir=data_dir)
...
def run_all(request: Request, body: Optional[dict] = None):
    ...
    ext_values = _load_ext_value_maps(repo, body.get("ext_columns"), data_dir=data_dir)
...
def limit_ladder(
    ...
    # The current ladder must use the selected provider's latest daily
    # snapshot. Persisted enriched data remains the source for historical
    ...
    if as_of is None:
        ...
        # An explicitly selected current date without a matching provider
        # snapshot is genuinely unavailable; do not silently show yesterday.
        live_snapshot = None
    if not as_of:
        raise HTTPException(status_code=400, detail="无可用数据日期")
    ...
    if live_snapshot is not None and live_date == as_of:
        ...
        df = svc._load_enriched_for_date(as_of)
    # compute_limit_signals uses instrument metadata for the calculation and
    # deliberately drops those helper columns. Reattach the canonical stock
    # name for the ladder response without changing any price fields.
    if "name" not in df.columns:
        try:
            instruments = repo.get_instruments_asset("stock")
            if instruments is not None and {"symbol", "name"}.issubset(instruments.columns):
                df = df.join(
                    instruments.select(["symbol", "name"]).unique(subset=["symbol"]),
                    on="symbol",
                    how="left",
                )
        except Exception as exc:  # noqa: BLE001
            logger.warning("limit ladder name lookup unavailable: %s", type(exc).__name__)
    if df.is_empty():
        return {
            "as_of": str(as_of),
            "data_freshness": {
    ...
    # A provider snapshot contains today's signal only. Extend it with the
    # persisted previous-day run length for an accurate current board count.
    current_consec = pl.max_horizontal(consec, prev_c + 1)
    df = df.with_columns([
        ...
        pl.when(is_limit).then(current_consec)
        .when(is_broken | is_failed).then(prev_c + 1)
        .otherwise(0).cast(pl.UInt32).alias("boards"),
        pl.when(is_limit).then(current_consec)
        .otherwise(consec).cast(pl.UInt32).alias(consec_col),
    ])
