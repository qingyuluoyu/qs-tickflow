...
def _daily_repair_allowed(capset) -> bool:
    """Allow historical repair when the selected provider supports daily batch pulls."""
    ...
    return provider != "tickflow" and custom_sources.provider_has_dataset(provider, "daily")


def _refresh_after_data_write(repo) -> None:
    """Refresh every process cache consumed by repaired market data."""
    repo.clear_cache()
    repo.refresh_cache()
...
async def extend_history(request: Request):
    ...
    try:
        ...
        if not _daily_repair_allowed(capset):
            raise HTTPException(status_code=403, detail="当前日线数据源不支持批量修复")
        ...
        async def task() -> None:
            ...
            try:
                ...
                if "error" in result:
                    ...
                    _refresh_after_data_write(repo)
