...
class DatasetConfigIn(BaseModel):
    ...
    batch: int | None = None
    rpm: int | None = None
    range_window_days: int | None = Field(default=None, ge=1, le=365)
    response_path: str = ""
...
class CustomSourceIn(BaseModel):
    ...
    datasets: dict[str, DatasetConfigIn] = Field(default_factory=dict)
...
def uninstall_plugin(name: str) -> dict:
    ...
    for getter, key, default in [
        ...
        if getter() == name:
            server_preferences.save({key: default})
    custom_sources.load_all()
    result = list_data_sources()
...
def update_realtime_quotes(req: RealtimeQuotesPrefs, request: Request) -> dict:
    ...
    if req.realtime_quotes_enabled and not allowed:
        # 当前档位不允许开启实时行情 — 强制关闭
        preferences.set_realtime_quotes_enabled(False)
        if qs:
            qs.disable()
    ...
    preferences.set_realtime_quotes_enabled(req.realtime_quotes_enabled)
    if qs:
...
def update_realtime_monitor_config(req: RealtimeMonitorConfigIn, request: Request) -> dict:
    ...
    if req.strategy_monitor_ids is not None or req.strategy_monitor_enabled is not None:
        ...
        if (monitor_engine is not None or user is not None) and strategy_engine is not None:
            ...
            try:
                ...
                # Legacy mode reloads the shared engine; account mode reloads
                # only the caller's personal runtime engine.
