...
def run(req: BacktestRequest, request: Request):
    ...
    svc = BacktestService(
        repo,
        data_dir=request_data_root(request, shared_root=settings.data_dir),
    )
...
def strategy_run(req: StrategyBacktestRequest, request: Request):
    ...
    task = make_worker_task(
        ...
        settings.data_dir,
        cfg,
        user_data_dir=request_data_root(request, shared_root=settings.data_dir),
    )
...
class _BacktestJob:
    ...
    def __init__(self, key: str, owner_id: str = "anonymous"):
        self.key = key
...
def _request_user_scope(request: Request) -> str:
    """Return the authenticated owner used to namespace in-memory jobs."""
    user = getattr(getattr(request, "state", None), "user", None)
    return str(getattr(user, "id", None) or "anonymous")


def _job_owned_by_request(job: _BacktestJob | None, request: Request) -> bool:
    return job is not None and job.owner_id == _request_user_scope(request)
...
def _make_job_key(
    ...
    scope_key: str = "anonymous",
) -> str:
    raw = f"{scope_key}|{strategy_id}|{symbols}|{start}|{end}|{matching}|{entry_fill}|{exit_fill}|{fees_pct}|{slippage_bps}|{max_positions}|{max_exposure_pct}|{initial_capital}|{position_sizing}|{params}|{overrides}|{mode}|{holding_days}|{commission_pct}|{stamp_tax_pct}|{asset_type}|{minute_fill}|{regime_filter}"
...
async def strategy_stream(
    ...
    job_key = _make_job_key(
        ...
        scope_key=_request_user_scope(request),
    )
    ...
    with _jobs_lock:
        ...
        if job is None:
            job = _BacktestJob(job_key, _request_user_scope(request))
            ...
            def _run_backtest():
                ...
                try:
                    task = make_worker_task(
                        ...
                        settings.data_dir,
                        cfg,
                        user_data_dir=request_data_root(
                            request,
                            shared_root=settings.data_dir,
                        ),
                    )
...
async def strategy_cancel(request: Request):
    ...
    with _jobs_lock:
        job = _running_jobs.get(job_key)
    if _job_owned_by_request(job, request) and not job.done:
...
def _make_opt_job_key(
    ...
    raw = (
        f"OPT|{scope_key}|{strategy_id}|{symbols}|{start}|{end}|{param_grid}|{objective}|"
...
def _make_wf_job_key(
    ...
    raw = (
        f"WF|{scope_key}|{strategy_id}|{symbols}|{start}|{end}|{param_grid}|{objective}|"
