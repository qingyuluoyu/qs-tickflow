...
def _latest_pipeline_coverage(jobs: list[dict[str, Any]]) -> dict[str, Any] | None:
    """Return the newest pipeline's structured market-data coverage report."""
    for job in jobs:
        result = job.get("result") or {}
        freshness = result.get("data_freshness")
        if not freshness:
            details = job.get("error_detail") or {}
            freshness = details.get("data_freshness")
        if not freshness:
            continue
        return {
            "job_id": job.get("id"),
...
def build_market_data_health(coverage: dict[str, Any] | None) -> dict[str, Any]:
    """Turn pipeline coverage into an actionable, non-empty health contract.

    Missing inactive/suspended instruments are expected exclusions; unresolved
    instruments remain degraded until the provider reconciliation explains them.
    The report intentionally exposes counts and dates rather than fabricating
    rows or silently treating a failed provider as an empty dataset.
    """
    freshness = (coverage or {}).get("data_freshness") or {}
    coverage_status = freshness.get("status") or "unavailable"
    unresolved = int(freshness.get("unresolved_symbol_count") or 0)
    missing = int(freshness.get("missing_symbol_count") or 0)
    inactive = int(freshness.get("inactive_symbol_count") or 0)
    ...
    if coverage_status == "unavailable" or not freshness.get("provider_date"):
        ...
        or unresolved > 0
        or reconciliation.get("status") == "unavailable"
    ):
        status = "degraded"
    elif coverage_status in {"ready", "partial"}:
        status = "ok"
    else:
        status = "degraded"
    ...
    return {
        ...
        "provider": freshness.get("provider") or freshness.get("source"),
        ...
        "checks": {
            ...
            "coverage": {
                "requested": int(freshness.get("requested_symbol_count") or 0),
                "available": int(freshness.get("available_symbol_count") or 0),
                "missing": missing,
                "inactive": inactive,
                "unresolved": unresolved,
            },
            "provider_reconciliation": {
                "status": reconciliation.get("status", "not_checked"),
                "symbols_checked": int(reconciliation.get("symbols_checked") or 0),
                "recent_history": int(reconciliation.get("symbols_with_recent_history") or 0),
                "historical_only": int(reconciliation.get("symbols_with_historical_only") or 0),
                "no_history": int(reconciliation.get("symbols_without_history") or 0),
                "without_recent_history": int(
                    reconciliation.get("symbols_without_recent_history") or 0
                ),
            },
        },
    }
...
def status(request: Request) -> dict:
    ...
    pipeline_coverage = _latest_pipeline_coverage(job_store.list_recent(limit=50))
    market_data_health = build_market_data_health(pipeline_coverage)
    ...
    "minute":      _get_table_stats("minute",      lambda: _safe_aggregate_minute(repo)),
        ...
        "market_data_coverage": pipeline_coverage,
...
@router.get("/health")
def market_data_health(request: Request) -> dict:
    """Return the latest real-market coverage health without triggering a sync."""
    from app.services.pipeline_jobs import job_store

    coverage = _latest_pipeline_coverage(job_store.list_recent(limit=50))
    report = build_market_data_health(coverage)
    report["checked_at"] = datetime.now(timezone.utc).isoformat(timespec="seconds").replace(
        "+00:00", "Z"
    )
    return report
