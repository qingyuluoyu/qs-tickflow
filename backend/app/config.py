...
class Settings(BaseSettings):
    ...
    # DeepSeek is the platform default. The deployment secret is injected
    # through the ignored shared secret store or environment, never committed.
    ...
    port: int = 3018
    log_level: str = "INFO"
    # Same-origin is the secure default. Set an explicit comma-separated list
    # only when a separately hosted trusted browser frontend is required.
    cors_origins: str = ""
    ...
    # Operator-only registration directory.  Empty means the hidden endpoint
    # is disabled; never fall back to a predictable default in production.
    qingshu101_admin_key: str = ""
    # Production must set QINGSHU101_HOST explicitly; empty keeps only local
    # development hosts available to the protected operator API.
    qingshu101_host: str = ""
    qingshu101_cookie_ttl: int = 8 * 60 * 60
    account_directory_refresh_interval: float = 30.0
