...
def make_worker_task(
    ...
    *,
    ...
    # Market parquet/SQLite stays in the shared read-only data root while
    # custom strategies and per-user overrides are resolved from the account
    # workspace.  Keeping this optional preserves callers that run outside
    # an authenticated request (CLI/tests) and therefore use one root.
    if user_data_dir is not None:
        task["user_data_dir"] = str(user_data_dir.resolve())
    return task
...
def _worker_entry(task: dict[str, Any], event_queue, cancel_event) -> None:
    ...
    try:
        ...
        user_data_dir = Path(task.get("user_data_dir") or data_dir)
        store = DataStore(data_dir)
        repo = KlineRepository(store)
        strategy_engine = StrategyEngine(
            strategy_dirs=_strategy_dirs(user_data_dir),
            override_loader=lambda sid: strategy_config.load_override(user_data_dir, sid),
        )
