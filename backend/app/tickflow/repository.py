...
class DataStore:
    ...
    def reset_connection(self) -> None:
        """Close cached parquet readers before a large partition rewrite."""
        self.db.close()
        self.db = duckdb.connect(database=":memory:")
        self._register_views()
...
class KlineRepository:
    ...
    def release_parquet_read_handles(self) -> None:
        """Release DuckDB readers that can lock parquet replacement on Windows."""
        with self._lock:
            self.store.reset_connection()
    ...
    def _atomic_write_parquet(df: pl.DataFrame, out: Path) -> None:
        ...
        # Windows may briefly retain a read handle for an existing parquet
        # partition. Retrying the final atomic replacement never exposes the
        # temporary file to readers and avoids a failed full-history repair.
