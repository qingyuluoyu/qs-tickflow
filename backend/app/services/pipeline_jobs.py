...
_process_run_lock_dir: Path | None = None


def _process_lock_path() -> Path:
    from app.config import settings
    return settings.data_dir / ".pipeline-run.lock"


def _windows_pid_exists(pid: int) -> bool:
    """Use the Win32 process table because ``os.kill(pid, 0)`` is unreliable on Windows."""
    import ctypes
...
def _lock_owner_is_alive(lock_dir: Path) -> bool:
    try:
        pid = int((lock_dir / "pid").read_text(encoding="ascii").strip())
        if os.name == "nt":
...
def _acquire_process_run_lock() -> Path | None:
    """跨进程获取数据写入锁；仅清理确认所属进程已退出的遗留锁。"""
    lock_dir = _process_lock_path()
    try:
        lock_dir.mkdir()
    except FileExistsError:
        if not _lock_owner_is_alive(lock_dir):
            try:
...
def try_acquire_run_slot() -> bool:
    """尝试占用重任务执行槽，覆盖同进程和共享数据目录的其他进程。"""
    global _process_run_lock_dir
    if not _heavy_run_lock.acquire(blocking=False):
        return False
    lock_dir = _acquire_process_run_lock()
    if lock_dir is None:
        _heavy_run_lock.release()
        return False
    _process_run_lock_dir = lock_dir
    return True
...
def release_run_slot() -> None:
    """释放重任务执行槽(允许跨线程释放)。"""
    global _process_run_lock_dir
    lock_dir = _process_run_lock_dir
    _process_run_lock_dir = None
    if lock_dir is not None:
        try:
            (lock_dir / "pid").unlink(missing_ok=True)
            lock_dir.rmdir()
        except OSError:
            logger.warning("failed to release process data lock %s", lock_dir)
    try:
        _heavy_run_lock.release()
    except RuntimeError:
        # 未持有(重复释放)—— 幂等忽略
