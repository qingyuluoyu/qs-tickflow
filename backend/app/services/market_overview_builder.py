...
def _dashboard_daily_snapshot(repo) -> tuple[date | None, pl.DataFrame | None]:
    """Read an unfiltered current snapshot for the dashboard only.

    The regular screener/history path intentionally remains backed by the
    persisted enriched cache.  When the selected provider is TeaJoin, the
    dashboard additionally asks its daily endpoint for the latest complete
    ...
    try:
        ...
        # metadata still reports its date.
        import logging

        logging.getLogger(__name__).warning(
            "dashboard daily snapshot unavailable (provider=%s): %s",
            provider_name,
            type(exc).__name__,
        )
        return None, None

    if snapshot is None or snapshot.is_empty() or "date" not in snapshot.columns:
        return None, None
    ...
    if latest > target_date:
        ...
        logging.getLogger(__name__).warning(
            "dashboard daily snapshot is ahead of local market date: %s",
            latest,
        )
        return None, None

    # Keep the quote labels from the existing instrument master, but never
    # reuse its prices or daily metrics.  TeaJoin may include a nullable name
    # column, so fill missing labels rather than only joining when the column
    # is absent; the four dashboard ranking lists must always be readable.
    if repo is not None:
        try:
            instruments = None
            if hasattr(repo, "get_instruments"):
                instruments = repo.get_instruments()
            if (instruments is None or instruments.is_empty()) and hasattr(repo, "get_instruments_asset"):
            ...
            if (
                ...
                labels = instruments.select(["symbol", "name"]).unique(subset=["symbol"])
                if "name" in snapshot.columns:
                    snapshot = snapshot.join(
                        labels.rename({"name": "_instrument_name"}),
                        on="symbol",
                        how="left",
                    ).with_columns(
                        pl.when(
                    ...
                    snapshot = snapshot.join(labels, on="symbol", how="left")
        except Exception:
            pass
...
def _merge_live_snapshot_indicators(
    ...
    """Add indicators only from the same-date local enriched snapshot.

    TeaJoin's daily table is the source of truth for today's OHLCV/change
    fields.  Same-date locally computed indicators (moving averages, volume
    ratios, limit signals) are deterministic derivatives and are safe to
    reuse.  A date mismatch is rejected so a newer provider snapshot cannot
    inherit yesterday's indicators.
    """
    if snapshot.is_empty() or enriched.is_empty() or "symbol" not in enriched.columns:
        return snapshot
    if "date" in enriched.columns:
        dates = enriched.get_column("date").drop_nulls().unique().to_list()
        if dates and any(value != as_of for value in dates):
            return snapshot
    ...
    extra = [
        column for column in indicator_cols
        if column in enriched.columns and column not in snapshot.columns
    ]
    name_needed = "name" in enriched.columns and (
        "name" not in snapshot.columns
        or snapshot.get_column("name").null_count() > 0
    )
    if not extra and not name_needed:
        return snapshot
    right_columns = ["symbol", *extra]
    if name_needed:
        right_columns.append("name")
    right = enriched.select(right_columns).unique(subset=["symbol"], keep="last")
    if name_needed:
        right = right.rename({"name": "_enriched_name"})
    merged = snapshot.join(right, on="symbol", how="left")
    if name_needed:
        if "name" in snapshot.columns:
...
def _derive_dashboard_limit_indicators(
    ...
    """Compute limit-up/down flags from the live daily OHLCV snapshot."""
    if snapshot.is_empty() or repo is None:
        return snapshot
    try:
        instruments = repo.get_instruments_asset("stock")
        ...
        for source, target in aliases.items():
            if source in working.columns and target not in working.columns:
                working = working.with_columns(pl.col(source).alias(target))
        target_date = working.get_column("date").max() if "date" in working.columns else None
        if target_date is not None and "prev_close" in working.columns:
            previous = working.with_columns(
                pl.col("date").dt.offset_by("-1d").alias("date"),
                pl.col("prev_close").alias("close"),
                pl.col("prev_close").alias("raw_close"),
                pl.col("prev_close").alias("high"),
                pl.col("prev_close").alias("low"),
                pl.col("prev_close").alias("raw_high"),
                pl.col("prev_close").alias("raw_low"),
            )
            working = pl.concat([previous, working], how="diagonal_relaxed").sort(
                ["symbol", "date"]
            )
        result = compute_limit_signals(
            ...
            historical_shares=(
                repo.get_historical_shares()
                if hasattr(repo, "get_historical_shares")
                else None
            ),
        )
        return result.filter(pl.col("date") == target_date) if target_date is not None else result
    except Exception as exc:
        import logging

        logging.getLogger(__name__).warning(
            "dashboard limit indicators unavailable: %s",
            type(exc).__name__,
        )
        return snapshot
...
def _data_freshness(
    ...
    *,
    ...
    """Expose the source and freshness of the snapshot used by the overview.

    The overview is allowed to show the most recent persisted trading-day
    snapshot when the upstream realtime feed is empty, but it must not present
    that snapshot as today's live market.  This metadata gives the UI an
    explicit, source-specific state instead of inferring freshness from a
    ...
    # stale hard-coded holiday table. Persisted fallback data still uses the
    # weekend-aware weekday rule below.
    is_stale = bool(
        not explicit_as_of
...
def _dashboard_index_quotes(provider_name: str, target_date: date) -> list[dict]:
    """Load core index quotes from the selected provider's daily endpoint."""
    try:
        from app.data_providers import custom as custom_sources
        ...
        if not custom_sources.provider_has_dataset(provider_name, "daily"):
            return []
        provider = custom_sources.get_provider(provider_name)
        loader = getattr(provider, "get_daily", None)
        if not callable(loader):
            return []
        frame = loader(
            list(CORE_INDEX_SYMBOLS),
            start_time=datetime.combine(target_date - timedelta(days=10), time.min),
            end_time=datetime.combine(target_date, time.min),
            asset_type="index",
        )
    except Exception as exc:
        import logging
        ...
        logging.getLogger(__name__).warning(
            "dashboard index snapshot unavailable (provider=%s): %s",
            provider_name,
            type(exc).__name__,
        )
        return []
    ...
    for symbol in CORE_INDEX_SYMBOLS:
        history = (
            ...
            .select([c for c in ("date", "close") if c in frame.columns])
            .to_dicts()
        )
        if not history:
            continue
...
def _dimension_rank(rows: list[dict], repo, kind: str, limit: int = 5, level: int | None = None) -> dict:
    ...
    for config in store.load_all():
        ...
        for ext_row in _read_ext_rows(data_root, config, field):
            ...
            for value in _dimension_values(ext_row.get(field)):
                ...
                group_sources.setdefault(value, source_field)
    ...
    for name, by_symbol in groups.items():
        ...
        items.append({
            ...
            "source_field": group_sources.get(name),
            "leader": {
...
def build_market_overview(
    ...
    indices = _index_quotes(
        repo,
        quote_service,
        None if not explicit_as_of else as_of,
        dashboard_provider=(
    ...
    # Use the provider snapshot as the source of truth for current dashboard
    # prices/breadth.  Historical enriched indicators are still used whenever
