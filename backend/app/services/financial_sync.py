...
_REPORT_TABLES = {"income", "balance_sheet", "cash_flow"}
_REPORT_IDENTITY = (
...
def _fetch_table(
    ...
    if is_custom:
        ...
        try:
            ...
            logger.info(
                "sync_%s using custom provider: %s",
                table,
                getattr(provider, "name", preferences.get_financial_provider()),
            )
...
def _write_table(table: str, df: pl.DataFrame, data_dir: Path) -> int:
    logger.info("sync_%s _write_table called: empty=%s, has_symbol=%s", table, df.is_empty(), "symbol" in df.columns if not df.is_empty() else "N/A")
    if df.is_empty() or "symbol" not in df.columns:
        return 0

    # 写入 Parquet (全量覆盖)
    if table in _REPORT_TABLES and set(_REPORT_IDENTITY) <= set(df.columns):
        before = len(df)
        if "update_flag" in df.columns:
            df = (
                ...
                .with_columns(
                    ...
                    .fill_null(-1.0)
                    .alias("_revision_order")
                )
                .sort([*_REPORT_IDENTITY, "_revision_order"])
                .unique(subset=list(_REPORT_IDENTITY), keep="last")
                .drop("_revision_order")
            )
        else:
            df = df.unique(subset=list(_REPORT_IDENTITY), keep="last")
        if len(df) != before:
            logger.info("sync_%s collapsed %d superseded report revisions", table, before - len(df))
...
def _shares_from_custom_metrics(data_dir: Path, symbols: list[str]) -> pl.DataFrame:
    ...
    TeaJoin exposes share capital through the same daily_basic endpoint as
    metrics. A separate first-time ``shares`` sync would issue one historical
    request per security and needlessly multiply upstream cost and latency.
    The conversion retains every available trading-day record and converts the
    provider's ten-thousand-share unit to the canonical share unit.
    """
    if not _financial_is_custom():
        return pl.DataFrame()
    metrics = get_financial_df(data_dir, "metrics")
    required = {"symbol", "trade_date", "total_share", "float_share"}
    if metrics.is_empty() or not required <= set(metrics.columns):
        return pl.DataFrame()
    return (
        metrics
        ...
        .filter(
            ...
            & pl.col("total_share").is_not_null()
            & pl.col("float_share").is_not_null()
        )
        .select([
            pl.col("symbol"),
        ...
        .drop_nulls(["total_shares", "float_shares"])
        .unique(subset=["symbol", "period_end"], keep="last")
        .sort(["symbol", "period_end"])
    )
...
class FinancialScheduler:
    ...
    def __init__(self) -> None:
        ...
        self._schedule_interval_days = 7
    ...
    def start(
        ...
        *,
        ...
        self._capset = capset
        self._schedule_interval_days = max(1, min(30, int(schedule_interval_days)))
        if not capset.has(Cap.FINANCIAL) and not _financial_is_custom():
        ...
        try:
            while self._running:
                ...
                try:
                    ...
                    # event loop so authentication and other HTTP requests stay
                    # responsive while the scheduled pre-processing runs.
    ...
    def trigger(self, table: str | None = None) -> dict[str, int]:
        ...
        def _bg() -> None:
            logger.info("BACKGROUND THREAD STARTED for table=%s", table or "all")
            try:
