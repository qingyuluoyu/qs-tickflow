...
def _path() -> Path:
    ...
    p = personal_user_data_dir(settings.data_dir) / "preferences.json"
    return p
...
def get_realtime_quotes_enabled() -> bool:
    ...
    return bool(server_preferences.get("realtime_quotes_enabled", False))


def set_realtime_quotes_enabled(enabled: bool) -> bool:
    from app.services import server_preferences
    server_preferences.save({"realtime_quotes_enabled": bool(enabled)})
    return bool(enabled)
...
def get_realtime_quote_interval() -> float:
    from app.services import server_preferences
    return server_preferences.get_quote_interval(6.0)
...
def set_realtime_quote_interval(interval: float) -> float:
    """保存服务器行情轮询间隔 (不在此做 min/max 校验)。"""
    from app.services import server_preferences
    server_preferences.save({"realtime_quote_interval": interval})
    return interval
...
def get_daily_data_provider() -> str:
    ...
    default = "teajoin" if "teajoin" in allowed else "tickflow"
    provider = server_preferences.get_provider("daily", default)
    return provider if provider in allowed else default


def get_adj_factor_provider() -> str:
    from app.services import server_preferences
    provider = server_preferences.get_provider("adj_factor", "same_as_daily")
...
def get_minute_data_provider() -> str:
    ...
    allowed = _allowed_data_providers()
    default = "teajoin" if "teajoin" in allowed else "tickflow"
    provider = server_preferences.get_provider("minute", default)
    return provider if provider in allowed else default
...
def get_realtime_data_provider() -> str:
    ...
    allowed = _allowed_data_providers()
    default = "teajoin" if "teajoin" in allowed else "tickflow"
    provider = server_preferences.get_provider("realtime", default)
    return provider if provider in allowed else default
...
def get_financial_provider() -> str:
    ...
    allowed = _allowed_data_providers()
    default = "teajoin" if "teajoin" in allowed else "tickflow"
    provider = server_preferences.get_provider("financial", default)
    return provider if provider in allowed else default
...
def get_realtime_pull_stock() -> bool:
    from app.services import server_preferences
    return bool(server_preferences.get("realtime_pull_stock", True))


def get_realtime_pull_etf() -> bool:
    # 老用户兼容: ETF 实时默认关闭，避免升级后请求量/写盘量突然增加。
    from app.services import server_preferences
    return bool(server_preferences.get("realtime_pull_etf", False))


def get_realtime_pull_index() -> bool:
    from app.services import server_preferences
    return bool(server_preferences.get("realtime_pull_index", True))


def get_realtime_index_mode() -> str:
    from app.services import server_preferences
    mode = str(server_preferences.get("realtime_index_mode", "core") or "core").lower()
    return mode if mode in {"core", "all"} else "core"


def get_realtime_index_symbols() -> list[str]:
    from app.services import server_preferences
    stored = server_preferences.get("realtime_index_symbols", SIDEBAR_INDEX_SYMBOLS_DEFAULT)
