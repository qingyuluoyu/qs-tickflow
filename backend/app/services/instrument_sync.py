...
def _fetch_instruments_via_provider() -> list[dict] | None:
    ...
    if not custom_sources.is_custom_provider(provider_name):
        logger.error("selected provider %s is not available for instruments", provider_name)
        return []
    try:
        provider = custom_sources.get_provider(provider_name)
    except Exception as e:  # noqa: BLE001
        logger.warning("provider %s could not be loaded for instruments: %s", provider_name, e)
        return []
