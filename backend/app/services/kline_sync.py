...
def sync_and_persist_daily_batch(
    ...
    if provider_name != "tickflow":
        ...
        if custom_sources.provider_has_dataset(provider_name, "daily"):
            ...
            if df.is_empty():
                return 0
            # DuckDB views can retain parquet read handles on Windows. Release
            # them only after the provider has fully fetched and validated its
            # dataframe, immediately before the partition rewrite.
            ...
            return df.height
        logger.error("selected daily provider %s does not provide daily data", provider_name)
        return 0
...
def sync_adj_factor(symbols: list[str], repo: KlineRepository,
    ...
    if provider_name != "tickflow":
        ...
        logger.error("selected adj-factor provider %s does not provide adj_factor data", provider_name)
        return 0, []
    ...
    try:
        if not custom_sources.provider_has_dataset(provider_name, "minute"):
            try:
                ...
                return (None, True, None)
            if getattr(getattr(provider, "config", None), "fail_closed", False) is True:
                return (provider, False, "minute dataset is not configured")
            return (None, True, None)
        provider = custom_sources.get_provider(provider_name)
...
def _try_custom_minute(
    ...
    if fallback:
        ...
        return (None, True)
    if err is not None:
        logger.error("custom minute provider %s is fail-closed: %s", provider_name, err)
    ...
    try:
        ...
        if getattr(getattr(provider, "config", None), "fail_closed", False):
            logger.error("custom minute provider %s failed closed: %s", provider_name, e)
            return (pl.DataFrame(), False)
