...
def _fetch_instruments_by_type(instrument_type: str, asset_type_label: str) -> pl.DataFrame:
    ...
    if provider_name != "tickflow":
        ...
        if not custom_sources.provider_has_dataset(provider_name, "instruments"):
            logger.error(
                "selected provider %s does not provide %s instruments",
                provider_name,
                asset_type_label,
            )
            return pl.DataFrame()
        try:
            return custom_sources.get_provider(provider_name).get_instruments(asset_type_label)
        except Exception as e:  # noqa: BLE001
            logger.error(
                "custom provider %s failed to fetch %s instruments: %s",
                provider_name,
                asset_type_label,
                e,
            )
