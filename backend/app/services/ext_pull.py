...
def seconds_until_next_pull(
    config: ExtConfig,
    data_dir,
    *,
    now_ts: float | None = None,
) -> float:
    """Return the remaining refresh interval for an existing server snapshot.

    The server owns built-in market snapshots. Restarting a worker must not
    immediately redownload a still-fresh snapshot, otherwise login competes
    with duplicate network, parsing and parquet writes. Missing snapshots are
    preloaded immediately by returning zero.
    """
    if not config.pull:
        return 0
    part = data_dir / "ext_data" / config.id / "part.parquet"
    if not part.exists():
        return 0
    interval = max(config.pull.schedule_minutes * 60, 60)
    age = max(0.0, (time.time() if now_ts is None else now_ts) - part.stat().st_mtime)
