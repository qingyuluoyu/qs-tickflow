...
def _parse_share_date(column: pl.Expr, dtype: pl.DataType) -> pl.Expr:
    if dtype == pl.Utf8:
        value = column.cast(pl.Utf8).str.strip_chars()
        return value.str.strptime(pl.Date, format="%Y-%m-%d", strict=False).fill_null(
...
class ScreenerService:
    ...
    def _with_strategy_metadata(self, df: pl.DataFrame, target_date: date) -> pl.DataFrame:
        """Attach names and point-in-time share capital needed by strategy filters."""
        if df.is_empty():
            return df
        ...
        if not instruments.is_empty() and "symbol" in instruments.columns:
            missing = [
                ...
                if column in instruments.columns and column not in df.columns
            ]
            if missing:
                df = df.join(
                    instruments.select(["symbol", *missing]).unique(subset=["symbol"], keep="last"),
                    on="symbol",
                    how="left",
                )

        get_shares = getattr(self.repo, "get_historical_shares", None)
        shares = get_shares() if callable(get_shares) else pl.DataFrame()
        required = {"symbol", "period_end", "total_shares", "float_shares"}
        if shares.is_empty() or not required <= set(shares.columns):
            return df

        available_column = "announce_date" if "announce_date" in shares.columns else "period_end"
        available = _parse_share_date(pl.col(available_column), shares.schema[available_column])
        period_end = _parse_share_date(pl.col("period_end"), shares.schema["period_end"])
        candidates = (
            shares
            .select(
                pl.col("symbol").cast(pl.Utf8),
                available.alias("_share_available_date"),
                period_end.alias("_share_period_end"),
                pl.col("total_shares").cast(pl.Float64, strict=False).alias("_share_total_shares"),
                pl.col("float_shares").cast(pl.Float64, strict=False).alias("_share_float_shares"),
            )
            .filter(
                pl.col("symbol").is_not_null()
                & pl.col("_share_available_date").is_not_null()
                & (pl.col("_share_total_shares") > 0)
                & (pl.col("_share_float_shares") > 0)
            )
        ...
        # History rows need their own point-in-time share metadata; using the
        # target date for every warmup row would leak later share changes into
        # the strategy signal calculation.
        if "date" in df.columns:
            candidates = candidates.unique(
                subset=["symbol", "_share_available_date"], keep="last"
            ).sort(["symbol", "_share_available_date"])
            df = (
                df.with_columns(
                    pl.col("symbol").cast(pl.Utf8),
                    pl.col("date").cast(pl.Date, strict=False).alias("_metadata_date"),
                )
                .sort(["symbol", "_metadata_date"])
                .join_asof(
                    candidates,
                    left_on="_metadata_date",
                    right_on="_share_available_date",
                    by="symbol",
                    strategy="backward",
                    check_sortedness=False,
                )
                .drop(["_metadata_date", "_share_available_date", "_share_period_end"])
            )
        else:
            latest = (
                candidates
                .filter(pl.col("_share_available_date") <= pl.lit(target_date))
                .unique(subset=["symbol"], keep="last")
                .select(["symbol", "_share_total_shares", "_share_float_shares"])
            )
            if latest.is_empty():
                return df
    ...
    def _load_enriched_for_date(self, target_date: date) -> pl.DataFrame:
        ...
        if cache is not None and not cache.is_empty() and cache_date == target_date:
            return self._with_strategy_metadata(cache, target_date)
        ...
        if self.asset_type == "stock":
            ...
            if cached_hist is not None and not cached_hist.is_empty() and "date" in cached_hist.columns:
                ...
                if not df.is_empty():
                    ...
                    return self._with_strategy_metadata(df, target_date)
        ...
        return self._with_strategy_metadata(df_full, target_date)
    ...
    def _compute_enriched_full(self, df_target: pl.DataFrame, target_date: date) -> pl.DataFrame:
        ...
        return self._with_strategy_metadata(df_result, target_date)
    ...
    def _load_enriched_history(self, target_date: date, lookback_days: int) -> pl.DataFrame:
        ...
        if self.asset_type == "stock":
            ...
            if cached is not None and not cached.is_empty():
                ...
                cached = self._with_strategy_metadata(cached, target_date)
        ...
        if ttl_cached is not None:
            ...
            if now - ts < _HISTORY_CACHE_TTL:
                ...
                return self._with_strategy_metadata(cached_df, target_date)
        ...
        df_full = self._with_strategy_metadata(df_full, target_date)
