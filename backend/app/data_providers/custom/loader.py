...
def close_all() -> None:
    ...
    for provider in providers:
        try:
            ...
            logger.warning("custom data source close failed: %s", getattr(provider, "name", "unknown"), exc_info=True)
...
def _config_to_dict(config: CustomSourceConfig) -> dict:
    ...
    for ds_name, ds in config.datasets.items():
        out["datasets"][ds_name] = {
            ...
            **({"range_window_days": ds.range_window_days} if ds.range_window_days is not None else {}),
            ...
            **({
                ...
                "financial_url_template": ds.financial_url_template,
                "financial_table_map": dict(ds.financial_table_map),
                "daily_url_by_asset_type": dict(ds.daily_url_by_asset_type),
                "instruments_url_by_asset_type": dict(ds.instruments_url_by_asset_type),
                "instruments_params_by_asset_type": dict(ds.instruments_params_by_asset_type),
                "instruments_body_by_asset_type": dict(ds.instruments_body_by_asset_type),
            ...
            **({"symbols_body_path": ds.symbols_body_path} if ds_name == "realtime" and ds.symbols_body_path else {}),
        }
    return out
...
def _sanitize_dataset(ds_name: str, ds_cfg: dict) -> dict:
    ...
    if ds_cfg.get("timeout") is not None:
        ...
        if timeout != DEFAULT_TIMEOUT:
            out["timeout"] = timeout
    if ds_cfg.get("range_window_days") is not None:
        try:
            range_window_days = int(ds_cfg["range_window_days"])
        except (TypeError, ValueError) as e:
            raise ValueError(f"{ds_name}: range_window_days must be an integer between 1 and 365") from e
        if not 1 <= range_window_days <= 365:
            raise ValueError(f"{ds_name}: range_window_days must be between 1 and 365")
        out["range_window_days"] = range_window_days
    ...
    if ds_name == "adj_factor":
        kind = str(ds_cfg.get("adj_factor_kind", "event_ratio") or "event_ratio")
        if kind in {"event_ratio", "cumulative"}:
            out["adj_factor_kind"] = kind
    if ds_name == "realtime":
        symbols_body_path = str(ds_cfg.get("symbols_body_path") or "").strip()
        if symbols_body_path:
            out["symbols_body_path"] = symbols_body_path
    if ds_name not in {"realtime", "instruments"}:
        ...
        for key in (
            ...
            if value:
                out[key] = value
        if bool(ds_cfg.get("date_only", False)):
            out["date_only"] = True
        financial_url_template = str(ds_cfg.get("financial_url_template") or "").strip()
        if financial_url_template:
            out["financial_url_template"] = financial_url_template
        financial_table_map = {
            str(key): str(value)
            for key, value in (ds_cfg.get("financial_table_map") or {}).items()
            if str(key).strip() and str(value).strip()
        }
        if financial_table_map:
            out["financial_table_map"] = financial_table_map
        daily_url_by_asset_type = {
            str(key): str(value)
            for key, value in (ds_cfg.get("daily_url_by_asset_type") or {}).items()
            if str(key).strip() and str(value).strip()
        }
        if daily_url_by_asset_type:
            out["daily_url_by_asset_type"] = daily_url_by_asset_type
        instruments_url_by_asset_type = {
            str(key): str(value)
            for key, value in (ds_cfg.get("instruments_url_by_asset_type") or {}).items()
            if str(key).strip() and str(value).strip()
        }
        if instruments_url_by_asset_type:
            out["instruments_url_by_asset_type"] = instruments_url_by_asset_type
        instruments_params_by_asset_type = {
            str(key): dict(value)
            for key, value in (ds_cfg.get("instruments_params_by_asset_type") or {}).items()
            if str(key).strip() and isinstance(value, dict)
        }
        if instruments_params_by_asset_type:
            out["instruments_params_by_asset_type"] = instruments_params_by_asset_type
        instruments_body_by_asset_type = {
            str(key): dict(value)
            for key, value in (ds_cfg.get("instruments_body_by_asset_type") or {}).items()
            if str(key).strip() and isinstance(value, dict)
        }
        if instruments_body_by_asset_type:
            out["instruments_body_by_asset_type"] = instruments_body_by_asset_type
    if ds_name == "minute":
